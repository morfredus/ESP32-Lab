...
ESP32-Lab est un outil **non destructif** :

- il ne modifie jamais la carte (pas d'écriture Flash, pas de changement d'eFuse) ;
- la lecture de la table de partitions se fait en **lecture seule** ;
- aucune donnée n'est effacée automatiquement.

Tu peux donc explorer sans crainte d'abîmer ta carte.
...
| **MAC** | Identifiant matériel unique de la carte (ex. `80:b5:4e:d9:63:4c`). |
